...
struct AddressService {
    ...
    func getAddressesForTurf(selectedTurfId: Int, callback: (([Address]?, Bool, APIError?) -> Void)) {
        
        //api.get("addresses", parameters: ["lat": "44.9206935", "longitude": "-73.1290229", "radius": radius]) { (data, success, error) in
        api.get("addressesForTurf/\(selectedTurfId)", parameters: [:]) { (data, success, error) in
